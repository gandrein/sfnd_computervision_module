...
void computeTTCCamera(std::vector<cv::KeyPoint> &kptsPrev, std::vector<cv::KeyPoint> &kptsCurr,
  ...
  double median = distRatios[distRatios.size() / 2];
  TTC = -dT / (1 - median);
}

int main() {
  vector<cv::KeyPoint> kptsSource, kptsRef;
  readKeypoints("../data/C23A5_KptsSource_AKAZE.dat", kptsSource);
  readKeypoints("../data/C23A5_KptsRef_AKAZE.dat", kptsRef);

  vector<cv::DMatch> matches;
  readKptMatches("../data/C23A5_KptMatches_AKAZE.dat", matches);

  // readKeypoints("./data/C23A5_KptsSource_SHI-BRISK.dat"
  // readKeypoints("./data/C23A5_KptsRef_SHI-BRISK.dat"

  // readKptMatches("./data/C23A5_KptMatches_SHI-BRISK.dat", matches);
  double ttc;
