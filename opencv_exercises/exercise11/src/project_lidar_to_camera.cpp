...
void loadCalibrationData(cv::Mat &P_rect_00, cv::Mat &R_rect_00, cv::Mat &RT) {
  // ANG: these should be read from the KITTI txt file
...
void projectLidarToCamera2() {
  ...
  for (auto it = lidarPoints.begin(); it != lidarPoints.end(); ++it) {
    /* Filter lidar points to remove the ones from the back of the vehicle
     * as seen by the 360 deg rotating lidar. Also exclude the side and ground points.
     */
     ...
     * We just need to dived by the 3rd resulting homgeneous coordinate
     */
    cv::Point pt;
