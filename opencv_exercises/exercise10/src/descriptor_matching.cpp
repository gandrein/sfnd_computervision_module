...
void matchDescriptors(cv::Mat &imgSource, cv::Mat &imgRef, vector<cv::KeyPoint> &kPtsSource,
  ...
  if (matcherType.compare("MAT_BF") == 0) {
    ...
    if (descSource.type() != CV_32F) {  // OpenCV bug workaround : convert binary descriptors to floating point due to a
  ...
  if (selectorType.compare("SEL_NN") == 0) {  // nearest neighbor (best match)
    ...
    // Exercise assignment: implement k-nearest-neighbor matching
    std::vector<std::vector<cv::DMatch>> knn_matches;
    int desired_num_matches = 2;
    double t = (double)cv::getTickCount();
    matcher->knnMatch(descSource, descRef, knn_matches, desired_num_matches);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << " (KNN) with n=" << knn_matches.size() << " matches in " << 1000 * t / 1.0 << " ms" << endl;

    // Exercise assignment: filter matches using descriptor distance ratio test
  ...
  cv::drawMatches(imgSource, kPtsSource, imgRef, kPtsRef, matches, matchImg, cv::Scalar::all(-1), cv::Scalar::all(-1),
                  mask, cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
...
int main() {
  ...
  readKeypoints("../data/C35A5_KptsSource_BRISK_large.dat", kptsSource);
  readKeypoints("../data/C35A5_KptsRef_BRISK_large.dat", kptsRef);

  cv::Mat descSource, descRef;
  readDescriptors("../data/C35A5_DescSource_BRISK_large.dat", descSource);
  readDescriptors("../data/C35A5_DescRef_BRISK_large.dat", descRef);
