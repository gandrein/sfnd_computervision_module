...
void detKeypoints1() {
  ...
  // distance measure introduced in the course
  // assume no overlap between corners, then the max number of corners = numPixels/blockSize
  // assume full overlap between corners, then the max number of corners is = numPixels (also useful to avoid division
  // by zero)
  ...
  //   cv::FAST(img, kptsFast, threshold, setNMS, type); // this does not work, or at least not when called like this

  t = static_cast<double>((cv::getTickCount() - tick)) / cv::getTickFrequency();
